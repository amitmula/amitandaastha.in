			...
			if($data = get_option("WpFastestCachePreLoad")){
				...
				if(count($urls) > 0){
					foreach ($urls as $key => $arr) {
						...
						$response = wp_remote_get($arr["url"], array('timeout' => 10, 'headers' => array("cache-control" => array("no-store, no-cache, must-revalidate", "post-check=0, pre-check=0"),'user-agent' => $user_agent)));
