...
class WpvTemplates {
		...
		if ( $total_pages > 1 ) {
			...
			echo paginate_links(
				array(
					...
					'total' => $total_pages,
					'prev_text' => __( 'Prev', 'honeymoon' ),
					'next_text' => __( 'Next', 'honeymoon' ),
				)
			);
			// xss ok
	...
	public static function pagination( $pagination_type = null ) {
		...
		if ( $pagination_type == 'paged' ) {
			...
			if ( (int)$max > (int)$paged ) {
				...
				echo '<div class="load-more"><a href="'.esc_attr( $url ).'" class="'.$class.'"><span>'.__( 'Load more', 'honeymoon' ).'</span></a></div>';
			}
		}
	}
	...
	public static function custom_link_pages( $args = '' ) {
		$defaults = array(
			'before' => '<p id="post-pagination">' . __( 'Pages:', 'honeymoon' ),
			'after' => '</p>',
			'text_before' => '',
			'text_after' => '',
			'next_or_number' => 'number',
			'nextpagelink' => __( 'Next page', 'honeymoon' ),
			'previouspagelink' => __( 'Previous page', 'honeymoon' ),
			'pagelink' => '%',
			'echo' => 1,
		);
